...
export function AssignmentOverviewPanel({
  ...
  const missingTargetCount = repos.filter((repo) => !repo.baseSha || !repo.submissionSha).length;
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-wrap gap-2 border-t border-zinc-300 bg-white px-4 py-3">
        <div className="rounded-none border border-zinc-200 bg-zinc-50 px-3 py-2 text-sm text-zinc-600">
          PRs prepared: <span className="font-semibold text-zinc-900">{preparedPrCount}</span>
        </div>
        <Button type="button" size="sm" variant="secondary" onClick={onOpenQueue}>
          Open queue tab
        </Button>
        <Button type="button" size="sm" variant="outline" onClick={onOpenSummary}>
          Open review summary
        </Button>
      </div>
    </PanelShell>
